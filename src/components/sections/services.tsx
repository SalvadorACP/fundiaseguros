import { CarFront, HeartPulse, ShieldCheck } from "lucide-react";
  ...
  return (
    <section id="seguros" className="bg-primary px-6 py-20 text-primary-foreground sm:px-10 sm:py-24 lg:px-16">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-12 grid gap-px overflow-hidden rounded-2xl bg-white/15 md:grid-cols-3">
          {services.map(({ icon: Icon, title, description }, index) => (
            <Link href={`/cotizar/${["auto", "gastos-medicos", "vida"][index]}`} key={title} className="group bg-primary p-7 transition-colors hover:bg-white/[0.04] focus-visible:outline-2 focus-visible:outline-accent focus-visible:-outline-offset-4 sm:p-8">
              ...
              <span className="mt-6 block font-semibold text-accent">Solicitar cotización →</span>
            </Link>
          ))}
        </div>
      </div>
